...
export default (env = {}, argv) => {
  ...
  return {
    ...
    plugins: [
      ...
      new DefinePlugin({
        __FMH_APP_NAME__: JSON.stringify('insurance-mfe'),
        __FMH_VERSION__: JSON.stringify(version),
        __FMH_ENV__: JSON.stringify(process.env.FMH_ENV ?? 'local'),
      }),
      new HtmlWebpackPlugin({
        templateContent: `<!doctype html><html><body style="background:#020617;color:#fff;font-family:system-ui"><h1>insurance-mfe ${release} online</h1><p>Version <strong>${version}</strong> · port ${port}</p><p>Consume <code>remoteEntry.js</code> through the Shell.</p></body></html>`,
      }),
    ],
    devServer: {
      port,
